                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb text-right">
                                    ...
                                    <li class="active">Student Courses</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                ...
                <section id="main-content">
                    <div class="card">
                        <div class="card-header m-b-20">
                           <div class="card-header m-b-20">
								<div class="row">
									<form action="<?=site_url('vlrn-student-course');?>" method="post">
										<div class="col-md-4">
											<input type="text" class="form-control" name="email_id" placeholder="Email ID" value="<?=$email_id;?>">
										</div>
										<div class="col-md-4">
							...
							<table class="table f-s-13">
								...
								<tbody>
									...
									<tr>
                                        ...
                                        <td><?=$course_details[$crs->course_code]->title;?></td>
                                        <td><?= date('Y-m-d H:i:s',strtotime($crs->last_update)); ?></td>
										<td><?=isset($user_info[$crs->update_by])?$user_info[$crs->update_by]->f_name:'undefine';?></td>
										<td id="course-<?= $crs->course_code; ?>"><a href="#" data-email_id="<?=$email_id;?>" data-code="<?=$crs->course_code;?>" class="change-status badge badge-primary">Removed</a></td>
                                    </tr>
									<?php } ?>
								</tbody>
							</table>
			...
			<div class="modal-body">
				<div class="row">
					<div class="col-sm-12 attach-details">
						<form class="form-submit" method="post" action="<?= site_url('vlrn/add_vlrn_student_course'); ?>">
							<div class="row">
								<div class="col-md-6 col-sm-12 form-group" >
									<input type="text" name="email_id" class="form-control" value="<?=$email_id;?>" placeholder="Email ID">
								</div>
								<div class="col-md-6 col-sm-12 form-group" >
    ...
    <script>
        $( function() {
			...
			$('.change-status').on('click', function(){
				...
				if(email_id != "" && code != "")
				{
					$("#course-"+code).html("<img src='<?= base_url('assets'); ?>/images/loader.gif' style='width: 40px;'>");
					$.ajax({
						type: "POST",
						url: "<?= site_url('vlrn/remove_vlrn_active_course'); ?>",
						data: {email_id:email_id,code:code},
						dataType: 'json',
						cache: false,
