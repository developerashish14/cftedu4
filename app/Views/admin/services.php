                        ...
                        <div class="card-body">
                            <table class="table f-s-13">
                                ...
                                <tbody>
									...
									<form class="form-submit" action="<?= site_url('gadget/update_service'); ?>" method="post">
										<tr>
											<td><?= $i++; ?></td>
											<td><textarea name="description" class="form-control" width="200px"><?= $srv->description; ?></textarea></td>
											<td><input type="text" name="name" value="<?= $srv->name; ?>" ></td>
											<td><input type="text" name="fa_icon" value="<?= $srv->fa_icon; ?>" ></td>
											<td><?= $srv->last_update; ?></td>
											<td><?= $user_info[$srv->update_by]->f_name; ?></td>
											<td id="service-<?=$srv->id?>"><?= ($srv->status == 'D')?'<a href="#" data-id="'.$srv->id.'" data-status="'.$srv->status.'" class="change-status badge badge-primary">ACTIVE</a>':'<a href="#" data-id="'.$srv->id.'" data-status="'.$srv->status.'" class="change-status badge badge-danger">DEACTIVE</a>'; ?></td>
											<td>
												<input type="hidden" class="form-control" name="service_id" value="<?= $srv->id; ?>">
					...
					<div id="mediaadd" class="tab-pane fade in active">
						<div class="row">
							<div class="col-sm-12 attach-details">
								<form class="form-submit" method="post" action="<?= site_url('gadget/add_service')?>">
									<div class="row">
    ...
    <script>
        $( function() {
			...
			$('.change-status').on('click', function(){
				...
				if(item != "" && status != "")
				{
					$("#service-"+item).html("<img src='<?= base_url('assets'); ?>/images/loader.gif' style='width: 40px;'>");
					$.ajax({
						type: "POST",
						url: "<?= site_url('gadget/change_service_status'); ?>",
